...
def select_few_shot_examples(
    train_traces: List[Dict[str, Any]], num_positive: int = 1, num_negative: int = 3
) -> List[Dict[str, Any]]:
    ...
    console.print(
        f"[green]Selected {len(selected_examples)} few-shot examples ({len([e for e in selected_examples if e['label'] == 'PASS'])} PASS, {len([e for e in selected_examples if e['label'] == 'FAIL'])} FAIL)"
    )
