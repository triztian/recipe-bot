...
class AgentRetrievalEvaluator(BaseRetrievalEvaluator):
    ...
    def __init__(
        self, retriever, processed_queries: Dict[str, List[Dict[str, str]]] = None
    ):
        """
    ...
    def evaluate_all_queries_with_strategy(
        ...
        if len(processed_query_list) != len(queries):
            raise ValueError(
                f"Mismatch: {len(queries)} original queries vs {len(processed_query_list)} processed"
            )

        print(f"Evaluating {len(queries)} queries with {strategy} strategy...")

        results = []
        for i, (query_data, processed_query_data) in enumerate(
            zip(queries, processed_query_list)
        ):
            ...
            reciprocal_rank = self.calculate_reciprocal_rank(
                retrieved_ids, target_recipe_id
            )
            ...
            evaluation_result = {
                ...
                "retrieved_names": [
                    recipe["name"] for recipe in retrieval_results[:top_k]
                ],
                ...
                "bm25_scores": [
                    recipe.get("bm25_score", 0.0)
                    for recipe in retrieval_results[:top_k]
                ],
            }
...
def main():
    ...
    baseline_evaluator.print_detailed_results(
        baseline_results, show_failures=True, max_examples=3
    )
    baseline_evaluator.save_results(
        baseline_results, results_baseline_path, experiment_name="baseline_bm25"
    )
    ...
    print(
        f"📊 Processing speed: {len(query_strings) * 3 / processing_time:.1f} queries/second"
    )
    ...
    for strategy in strategies:
        ...
        start_time = time.time()
        results = enhanced_evaluator.evaluate_all_queries_with_strategy(
            queries, strategy, top_k=5
        )
        ...
        print(
            f"   Recall@5: {metrics['recall_at_5']:.3f} ({metrics['recall_at_5']*100:.1f}%)"
        )
    ...
    enhanced_evaluator.print_detailed_results(
        enhanced_results, show_failures=True, max_examples=3
    )
    enhanced_evaluator.save_results(
        enhanced_results,
        results_enhanced_path,
    ...
    print(
        f"   Parallel efficiency: {len(queries) * 3 / total_processing_time:.1f} queries/second"
    )
    ...
    for baseline_result, enhanced_result in zip(baseline_results, enhanced_results):
        if baseline_result["recall_5"] == 0.0 and enhanced_result["recall_5"] == 1.0:
            ...
            if helped_count <= 3:  # Show first 3 examples
                ...
                print(
                    f"   Enhanced: '{enhanced_result['search_query']}' ({enhanced_result['processing_strategy']})"
                )
                print(f"   Target: {enhanced_result['target_recipe_name']}")
                print(
                    f"   Result: ❌ → ✅ (Found at rank {enhanced_result['target_rank']})"
                )
    ...
    if improvement > 10:
        ...
        print(
            f"   • Processing overhead: {total_processing_time:.1f}s for {len(queries)} queries"
        )
    elif improvement > 5:
