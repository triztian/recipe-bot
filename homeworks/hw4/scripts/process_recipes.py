...
def process_recipe(row: Dict[str, str]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        n_ingredients = (
            int(row.get("n_ingredients", 0)) if row.get("n_ingredients") else 0
        )
        ...
        recipe = {
            ...
            "contributor_id": (
                int(row.get("contributor_id", 0)) if row.get("contributor_id") else 0
            ),
            "full_text": full_text,
        }
...
def load_and_process_recipes(
    csv_path: Path, max_recipes: Optional[int] = None
) -> List[Dict[str, Any]]:
...
def print_sample_recipes(recipes: List[Dict[str, Any]], n_samples: int = 3) -> None:
    ...
    for i, recipe in enumerate(recipes[:n_samples]):
        ...
        print(
            f"  Ingredients ({len(recipe['ingredients'])}): {recipe['ingredients'][:3]}..."
        )
        print(
            f"  Steps ({len(recipe['steps'])}): {recipe['steps'][0][:50] if recipe['steps'] else 'None'}..."
        )
...
def select_longest_recipes(
    recipes: List[Dict[str, Any]], top_n: int = 200
) -> List[Dict[str, Any]]:
    """Select the top N longest recipes by full text length."""
    print(
        f"Selecting top {top_n} longest recipes (by text length) from {len(recipes)} total recipes..."
    )

    # Sort recipes by full_text length (descending) and take top N
    sorted_recipes = sorted(
        recipes, key=lambda r: len(r.get("full_text", "")), reverse=True
    )
    ...
    if longest_recipes:
        ...
        avg_steps = sum(r.get("n_steps", 0) for r in longest_recipes) / len(
            longest_recipes
        )
        avg_ingredients = sum(r.get("n_ingredients", 0) for r in longest_recipes) / len(
            longest_recipes
        )
        avg_minutes = sum(
            r.get("minutes", 0) for r in longest_recipes if r.get("minutes", 0) > 0
        ) / len([r for r in longest_recipes if r.get("minutes", 0) > 0])

        print(f"Selected recipes stats:")
        print(
            f"  Text length range: {min(text_lengths)} - {max(text_lengths)} characters"
        )
        print(
            f"  Average text length: {sum(text_lengths) / len(text_lengths):.0f} characters"
        )
...
def main():
    ...
    print(
        f"Average steps per recipe: {sum(r['n_steps'] for r in longest_recipes) / len(longest_recipes):.1f}"
    )
    print(
