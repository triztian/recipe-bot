...
class RetrievalEvaluator(BaseRetrievalEvaluator):
    ...
    def analyze_by_query_characteristics(self, results: List[Dict[str, Any]]):
        ...
        if short_queries and long_queries:
            ...
            print(
                f"Short queries (≤8 words) Recall@5: {short_recall:.3f} ({len(short_queries)} queries)"
            )
            print(
    ...
    def print_final_summary(self, results: List[Dict[str, Any]]):
        ...
        print(
            f"   • Recall@1:  {final_metrics['recall_at_1']:.3f} ({final_metrics['recall_at_1']*100:.1f}%)"
        )
        print(
            f"   • Recall@3:  {final_metrics['recall_at_3']:.3f} ({final_metrics['recall_at_3']*100:.1f}%)"
        )
        print(
            f"   • Recall@5:  {final_metrics['recall_at_5']:.3f} ({final_metrics['recall_at_5']*100:.1f}%)"
        )
        print(
            f"   • Recall@10: {final_metrics['recall_at_10']:.3f} ({final_metrics['recall_at_10']*100:.1f}%)"
        )
        ...
        if final_metrics["average_rank_when_found"]:
            print(f"\n🎯 Ranking Analysis:")
            print(
                f"   • Average rank when found: {final_metrics['average_rank_when_found']:.2f}"
            )
            print(
                f"   • Median rank when found:  {final_metrics['median_rank_when_found']:.0f}"
            )
