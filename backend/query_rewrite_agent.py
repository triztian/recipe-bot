...
"""
    ...
    def _process_query_with_retry(
        self, query: str, strategy: str, max_retries: int = 3
    ) -> Dict[str, str]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    print(
                        f"Failed to process query after {max_retries} attempts: {query[:50]}..."
                    )
                    return {
                        "original_query": query,
    ...
    def batch_process_queries(
        self, queries: List[str], strategy: str = "rewrite"
    ) -> List[Dict[str, str]]:
        """
        ...
        print(
            f"Processing {len(queries)} queries with {strategy} strategy using {self.max_workers} workers..."
        )
    ...
    def batch_process_multiple_strategies(
        ...
        with ThreadPoolExecutor(max_workers=len(strategies)) as strategy_executor:
            strategy_futures = {
                strategy_executor.submit(
                    self.batch_process_queries, queries, strategy
                ): strategy
...
def main():
    ...
    start_time = time.time()
    multi_results = agent.batch_process_multiple_strategies(
        test_queries[:5]
    )  # Use fewer for demo
    elapsed = time.time() - start_time

    print(
        f"Processed {len(test_queries[:5])} queries with 3 strategies in {elapsed:.2f} seconds"
    )
