...
class BaseRetrievalEvaluator:
    ...
    def calculate_recall_at_k(
        self, retrieved_ids: List[int], target_id: int, k: int
    ) -> float:
    ...
    def calculate_reciprocal_rank(
        self, retrieved_ids: List[int], target_id: int
    ) -> float:
    ...
    def evaluate_single_query(
        ...
        evaluation_result = {
            ...
            "bm25_scores": [
                recipe.get("bm25_score", 0.0) for recipe in results[:top_k]
            ],
        }
    ...
    def calculate_aggregate_metrics(
        ...
        found_ranks = [
            r["target_rank"] for r in results if r["target_rank"] is not None
        ]
        ...
        metrics = {
            ...
            "average_rank_when_found": (
                statistics.mean(found_ranks) if found_ranks else None
            ),
    ...
    def print_detailed_results(
        ...
        print(
            f"Queries where target was found: {metrics['queries_found']}/{metrics['total_queries']}"
        )
        ...
        if successful:
            ...
            for i, result in enumerate(successful[:max_examples]):
                ...
                print(
                    f"   Retrieved #1: {result['retrieved_names'][0] if result['retrieved_names'] else 'None'}"
                )
